/* ENGLAND ENGINE — generated copy of script.js with Scotland specifics stripped. Edit script.js + regen, not here. */
/* =========================================================================
...
(function () {
  ...
  if (heroStats) T.stats.forEach(s => {
  ...
  function buildIllustratedMap() {
    if (!document.getElementById('illustrated-map')) return;
    const W = 1100, H = 800;
    const scot = T.route.slice();
  ...
  function buildLeaflet() {
    ...
    scot.forEach((s, i) => {
      ...
      L.marker([s.lat, s.lng], { icon }).addTo(map).bindPopup(
        `<b>${s.label}</b>${s.nights === 'drop-off' ? 'Drop-off' : s.nights === 'start' ? 'Trip start' : 'Nights ' + s.nights}<br/>
  ...
  function buildDriveBanner(d) {
    ...
    let dirBtn = '';
    if (isTransfer && hotel && d.transport !== 'train') {
    ...
    const headline = isTransfer ? `${d.transport === 'train' ? '\uD83D\uDE86' : '\uD83D\uDE97'} ${start.label} \u2192 ${d.overnight}` : `\uD83C\uDFE0 Based in ${d.overnight} \u2014 day-trip`;
    const stats = [
